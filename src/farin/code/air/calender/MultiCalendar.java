...
public class MultiCalendar extends Calendar {
    ...
    public MultiCalendar(TimeZone zone) {
        this(zone, Locale.getDefault());
    }

    public MultiCalendar(Locale aLocale) {
        this(TimeZone.getDefault(), aLocale);
    }

    public MultiCalendar(TimeZone zone, Locale aLocale) {

        super(zone, aLocale);
        timeZone = zone;
        Calendar calendar = Calendar.getInstance(zone, aLocale);

        YearMonthDate yearMonthDate = new YearMonthDate(calendar.get(YEAR), calendar.get(MONTH), calendar.get(DATE),calendar.get(DAY_OF_WEEK));
    ...
    public MultiCalendar(int year, int month, int dayOfMonth) {
        this(year, month, dayOfMonth, 0, 0, 0, 0);
    }

    public MultiCalendar(int year, int month, int dayOfMonth, int hourOfDay,
                          int minute) {
    ...
    MultiCalendar(int year, int month, int dayOfMonth,
        ...
        YearMonthDate yearMonthDate = jalaliToGregorian(new YearMonthDate(fields[1], fields[2], fields[5],get(DAY_OF_WEEK)));
    ...
    public static YearMonthDate gregorianToJalali(YearMonthDate gregorian) {
        ...
        return new YearMonthDate(jalaliYear, jalaliMonth, jalaliDay,gregorian.getDaynum());
    }
    ...
    public static YearMonthDate jalaliToGregorian(YearMonthDate jalali) {
        ...
        return new YearMonthDate(gregorianYear, gregorianMonth, gregorianDay,jalali.getDaynum());

    }

    public static YearMonthDate gregorianTohejri(YearMonthDate hejri) {  
        //Calendar today = Calendar.getInstance();  
        double day = hejri.getDate();
        double  month = hejri.getMonth();
        double  year = hejri.getMonth(); 
        ...
        return new YearMonthDate((int)myRes[2], (int)myRes[1], (int)myRes[0],hejri.getDaynum());  
    }  
    
    public static YearMonthDate gregorian()
    {
    	Calendar calendar=Calendar.getInstance();
    	return new YearMonthDate(calendar.get(YEAR), calendar.get(MONTH)
    			, calendar.get(DAY_OF_MONTH),calendar.get(DAY_OF_WEEK));
    }
    
    public static int weekOfYear(int dayOfYear, int year) {
        switch (dayOfWeek(MultiCalendar.jalaliToGregorian(new YearMonthDate(year, 0, 1,0)))) {
            case 2:
                dayOfYear++;
                break;
            case 3:
    ...
    protected void computeTime() {
        ...
        if (!isTimeSet && !isTimeSeted) {
            ...
            YearMonthDate yearMonthDate = jalaliToGregorian(new YearMonthDate(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH),cal.get(DAY_OF_WEEK)));
        ...
        switch (field) {
            ...
            case DST_OFFSET: {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) &&
                    ...
                    cal = new GregorianCalendar();
                    YearMonthDate yearMonthDate = jalaliToGregorian(new YearMonthDate(internalGet(YEAR), internalGet(MONTH), internalGet(DATE),cal.get(DAY_OF_WEEK)));
                    cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDate(), internalGet(HOUR_OF_DAY), internalGet(MINUTE),
                            internalGet(SECOND));
                    cal.set(field, value);
                    yearMonthDate = gregorianToJalali(new YearMonthDate(cal.get(YEAR), cal.get(MONTH), cal.get(DATE),cal.get(DAY_OF_WEEK)));
    ...
    protected void computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            //Day_of_Week
            super.set(DAY_OF_WEEK, dayOfWeek(jalaliToGregorian(new YearMonthDate(fields[1], fields[2], fields[5],get(DAY_OF_WEEK)))));
            //***
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            YearMonthDate yearMonthDate = jalaliToGregorian(new YearMonthDate(get(YEAR), get(MONTH), get(DATE),get(DAY_OF_WEEK)));
            ...
            gc.add(field, amount);
            yearMonthDate = gregorianToJalali(new YearMonthDate(gc.get(YEAR), gc.get(MONTH), gc.get(DATE),gc.get(DAY_OF_WEEK)));
            super.set(YEAR, yearMonthDate.getYear());
            super.set(MONTH, yearMonthDate.getMonth());
            super.set(DATE, yearMonthDate.getDate());
